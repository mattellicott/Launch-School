...
def find_best_square(brd, marker, squares_needed)
  FILE_DATA[:lines].each do |line|
    if match_logic?(brd, line, marker, 'line', squares_needed)
...
def find_best_corner_square(brd, marker)
  if match_logic?(brd, FILE_DATA[:corners], marker, 'corner')
    brd.each do |k, v|
...
def no_more_points_possible?(brd)
  FILE_DATA[:lines].each do |line|
    if (line_marker_count(brd, line, PLAYER_MARKER) == 4) ||
       (line_marker_count(brd, line, COMPUTER_MARKER) == 4)
      next
    end
    if (line_marker_count(brd, line, PLAYER_MARKER) +
        line_marker_count(brd, line, INITIAL_MARKER) == 4) ||
       (line_marker_count(brd, line, COMPUTER_MARKER) +
        line_marker_count(brd, line, INITIAL_MARKER) == 4)
      return false
    end
  end
...
def update_score!(brd, score)
  FILE_DATA[:lines].each do |line|
    score['Player'] += 2 if match_logic?(brd, line, PLAYER_MARKER, 'score')
    score['Computer'] += 2 if match_logic?(brd, line, COMPUTER_MARKER, 'score')
  end
  if match_logic?(brd, FILE_DATA[:corners], PLAYER_MARKER, 'score')
    score['Player'] += 1
  elsif match_logic?(brd, FILE_DATA[:corners], COMPUTER_MARKER, 'score')
    score['Computer'] += 1
  end
end

def match_logic?(brd, squares, marker, type = nil, squares_needed = nil)
  case type
  when 'line'
    brd.values_at(*squares).count(marker) == (4 - squares_needed) &&
      brd.values_at(*squares).count(INITIAL_MARKER) == squares_needed
  when 'corner'
    brd.values_at(*squares).count(marker) == 3 &&
      brd.values_at(*squares).count(INITIAL_MARKER) == 1
  when 'score'
    brd.values_at(*squares).count(marker) == 4
  end
end
